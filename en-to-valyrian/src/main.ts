...
const valyrian_textarea: HTMLTextAreaElement = document.querySelector("#hi-vl");
...
english_textarea.addEventListener("keyup", () => {
  throttle(() => {
    translate(english_textarea.value).then(
      (translation) => (valyrian_textarea.value = translation || "")
    );
  });
});

valyrian_textarea.addEventListener("focus", () => valyrian_textarea.blur());
